...
import { ShoppingBag, Search, User, Menu, X, Globe, Heart, Moon, Sun, LifeBuoy } from 'lucide-react';
...
const Header = () => {
    ...
    return (
        <>
            ...
            <header
                className={`fixed ${showAnnouncement ? 'top-8' : 'top-0'} left-0 w-full bg-white dark:bg-black z-50 transition-all duration-300 shadow-md border-b border-gray-100 dark:border-white/10`}
                onMouseLeave={() => setActiveCategory(null)}
            >
                {/* Mobile top strip: language left, auth/theme/search right */}
                <div className="md:hidden flex items-center justify-between px-4 h-10 bg-white dark:bg-black border-b border-gray-100 dark:border-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            onClick={toggleTheme}
                            className="p-1 rounded-md text-gray-500 dark:text-sky-200 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-50">
                    <div className="flex justify-between items-center h-16">
                        ...
                        <Link to="/" className="flex-shrink-0 flex items-center gap-2 group">
                            ...
                            />
                            <span className="ml-2 font-extrabold text-gray-900 dark:text-white hidden" aria-hidden>
                                5 Electro
                            </span>
                        </Link>
                        {/* Mobile: show only menu on right; desktop keeps full icons */}
                        ...
                        <div className="hidden md:flex items-center space-x-4 md:space-x-6">
                            ...
                            <button
                                onClick={toggleTheme}
                                className="hidden md:inline-flex p-2 rounded-full text-gray-500 dark:text-sky-200 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
                                aria-label="Toggle theme"
                            >
                            ...
                            <button
                                onClick={() => setIsSearchOpen(!isSearchOpen)}
                                className={`hidden md:inline-flex transition-colors p-2 rounded-full hover:bg-gray-100 ${isSearchOpen ? 'text-primary bg-gray-50' : 'text-gray-600'}`}
                                aria-label="Rechercher"
                            >
                    ...
                    <div className="absolute top-full left-0 w-full bg-white border-b border-gray-100 shadow-xl animate-fade-in p-4 z-40">
                        <form onSubmit={handleSearch} className="max-w-4xl mx-auto relative flex items-center">
                            ...
                            <input
                                ...
                                className="w-full bg-gray-50 border-none rounded-2xl py-4 pl-12 pr-4 text-lg font-medium outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-gray-400 text-black dark:text-black"
                                autoFocus
                            />
                            <button
                                type="submit"
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-100 shadow-lg h-[calc(100vh-6rem)] overflow-y-auto pt-6">
                ...
                {/* Assistant removed from header; assistant now available in Footer */}
